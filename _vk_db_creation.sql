...
ALTER TABLE vk.media_files_likes;
ADD CONSTRAINT media_files_likes_fk 
FOREIGN KEY (media_id) REFERENCES vk.media(id);

ALTER TABLE vk.media_files_likes; 
ADD CONSTRAINT media_files_likes_fk_1 
...
CREATE TABLE posts (
	post_id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT UNIQUE,
	user_id BIGINT UNSIGNED NOT NULL,
	post_text TEXT,
	publishing_date DATETIME DEFAULT NOW(),
...
CREATE TABLE post_likes (
	id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT UNIQUE, -- Сомневаюсь в необходимости создания ID в этом примере.
							   -- Возможно, было бы лучше создать составной ключ (user_id, post_id)
	user_id BIGINT UNSIGNED NOT NULL,
	post_id BIGINT UNSIGNED NOT NULL,

	CONSTRAINT post_likes_pk PRIMARY KEY (user_id, post_id),
	CONSTRAINT post_likes_fk1 FOREIGN KEY (user_id) REFERENCES users (user_id),
	CONSTRAINT post_likes_fk2 FOREIGN KEY (post_id) REFERENCES posts (post_id)
);

DROP TABLE IF EXISTS user_likes;
CREATE TABLE user_likes (
	id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT UNIQUE, -- Сомневаюсь в необходимости создания ID в этом примере.
							   -- Возможно, было бы лучше создать составной ключ (liked_user_id, liking_user_id)	
	liked_user_id BIGINT UNSIGNED NOT NULL,
	liking_user_id BIGINT UNSIGNED NOT NULL,

	CONSTRAINT user_likes_fk1 FOREIGN KEY (liking_user_id) REFERENCES users (user_id),
	CONSTRAINT user_likes_fk2 FOREIGN KEY (liked_user_id) REFERENCES users (user_id),
	CHECK (liking_user_id <> liked_user_id)
);
